#!/usr/bin/env python3
"""
Autocommit — keeps a git repository alive by committing and pushing on an
interval.

How it works:
- Reads a GitHub token from AUTOCOMMIT_GIT_TOKEN, GITHUB_TOKEN, or a
  token file (never hard-coded).
- Uses ~/auto as the live repository by default (configurable).
- Each cycle: fetch/reset to remote, copy this runner into the repo, update
  a heartbeat stamp block at the bottom of the copy, commit, push.
- A heartbeat stamp is only rewritten when the minute actually changed, so
  idle runs produce no empty commit noise.
"""
...
def _copy_runner(repo: Path, runner_path: Path) -> None:
...
# last-run-utc = 2026-09-16T19:56:25Z
# <<< AUTOCOMMIT-HEARTBEAT <<<
